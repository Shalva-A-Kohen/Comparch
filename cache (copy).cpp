...
//global
int bitmap_init_address = 0;
int bitmap_init_address2 = 0;
int bitmap1_start = 0;
int bitmap1_end = 0;
int bitmap2_start = 0;
int bitmap2_end = 0;
...
KNOB<UINT32> KnobDL1CacheSize(KNOB_MODE_WRITEONCE, "pintool",
    "dl1_c",  "64", "Cache size of DL1 in kilobytes");
...
// L1 bitmap data cache {{{
KNOB<bool>   KnobDLB1(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1",    "true", "Simulate DLB1?");
KNOB<UINT32> KnobDLB1NumWays(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1_a",  "4", "Cache associativity (1 for direct map) of DLB1");
KNOB<UINT32> KnobDLB1CacheSize(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1_c",  "32", "Cache size of DLB1 in kilobytes");
KNOB<string> KnobDLB1OutputInst(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1_oi", "cache.dlb1.inst.out", "Output file name for DLB1 (instruction)");
KNOB<string> KnobDLB1OutputAddr(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1_oa", "cache.dlb1.addr.out", "Output file name for DLB1 (address)");
KNOB<string> KnobDLB1OutputTime(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1_ot", "cache.dlb1.time.out", "Output file name for DLB1 (time)");
KNOB<string> KnobDLB1OutputLife(KNOB_MODE_WRITEONCE, "pintool",
    "dlb1_ol", "cache.dlb1.life.out", "Output file name for DLB1 (lifetime)");
// }}}
// L1 bitmap instruction cache {{{
KNOB<bool>   KnobILB1(KNOB_MODE_WRITEONCE, "pintool",
    "ilb1",    "false", "Simulate ILB1?");
KNOB<UINT32> KnobILB1NumWays(KNOB_MODE_WRITEONCE, "pintool",
    "ilb1_a",  "8", "Cache associativity (1 for direct map) of ILB1");
KNOB<UINT32> KnobILB1CacheSize(KNOB_MODE_WRITEONCE, "pintool",
    "ilb1_c",  "32", "Cache size of ILB1 in kilobytes");
KNOB<string> KnobILB1OutputInst(KNOB_MODE_WRITEONCE, "pintool",
    "ilb1_oi", "cache.ilb1.inst.out", "Output file name for ILB1 (instruction)");
KNOB<string> KnobILB1OutputTime(KNOB_MODE_WRITEONCE, "pintool",
    "ilb1_ot", "cache.ilb1.time.out", "Output file name for ILB1 (time)");
KNOB<string> KnobILB1OutputLife(KNOB_MODE_WRITEONCE, "pintool",
    "ilb1_ol", "cache.ilb1.life.out", "Output file name for ILB1 (lifetime)");
// }}}
// L2 bitmap cache {{{
KNOB<bool>   KnobULB2(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2",    "false", "Simulate LB2?");
KNOB<UINT32> KnobULB2NumWays(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2_a",  "8", "Cache associativity (1 for direct map) of LB2");
KNOB<UINT32> KnobULB2CacheSize(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2_c",  "256", "Cache size of ULB2 in kilobytes");
KNOB<string> KnobULB2OutputInst(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2_oi", "cache.ulb2.inst.out", "Output file name for LB2 (instruction)");
KNOB<string> KnobULB2OutputAddr(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2_oa", "cache.ulb2.addr.out", "Output file name for LB2 (address)");
KNOB<string> KnobULB2OutputTime(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2_ot", "cache.ulb2.time.out", "Output file name for LB2 (time)");
KNOB<string> KnobULB2OutputLife(KNOB_MODE_WRITEONCE, "pintool",
    "ulb2_ol", "cache.ulb2.life.out", "Output file name for LB2 (lifetime)");
// }}}
...
std::ofstream dlb1_output_inst;
std::ofstream dlb1_output_addr;
std::ofstream dlb1_output_time;
std::ofstream dlb1_output_life;

std::ofstream il1_output_inst;
std::ofstream il1_output_time;
std::ofstream il1_output_life;

std::ofstream ilb1_output_inst;
std::ofstream ilb1_output_time;
std::ofstream ilb1_output_life;
...
std::ofstream ulb2_output_inst;
std::ofstream ulb2_output_addr;
std::ofstream ulb2_output_time;
std::ofstream ulb2_output_life;
...
l1_cache::InfCache    *inf_dlb1 = 0;
l1_cache::OptCache *fa_opt_dlb1 = 0;
l1_cache::OptCache *sa_opt_dlb1 = 0;
l1_cache::LruCache *sa_lru_dlb1 = 0;

l1_cache::InfCache    *inf_ilb1 = 0;
l1_cache::OptCache *fa_opt_ilb1 = 0;
l1_cache::OptCache *sa_opt_ilb1 = 0;
l1_cache::LruCache *sa_lru_ilb1 = 0;
...
l2_cache::InfCache    *inf_ulb2 = 0;
l2_cache::OptCache *fa_opt_ulb2 = 0;
l2_cache::OptCache *sa_opt_ulb2 = 0;
l2_cache::LruCache *sa_lru_ulb2 = 0;
...
void LoadMultipleLines(ADDRINT inst_addr, ADDRINT addr, UINT32 size)
{
  ...
  if(use_bitmap) {
  if (KnobDLB1) {
       inf_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
    fa_opt_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
    sa_opt_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
    const bool is_dlb1_hit =
      sa_lru_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);

    if (KnobULB2) {
      if (!is_dlb1_hit) {
           inf_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
        fa_opt_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
        sa_opt_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
        sa_lru_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
      }
    }
  }
  }
...
void StoreMultipleLines(ADDRINT inst_addr, ADDRINT addr, UINT32 size)
{
  ...
  if(use_bitmap){
  if (KnobDLB1) {
       inf_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
    fa_opt_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
    sa_opt_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
    const bool is_dlb1_hit =
      sa_lru_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);

    if (KnobULB2) {
      if (!is_dlb1_hit) {
           inf_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
        fa_opt_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
        sa_opt_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
        sa_lru_ulb2->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
      }
    }
  }
  }
...
void LoadSingleLine(ADDRINT inst_addr, ADDRINT addr)
{
  ...
  if(use_bitmap){
  if (KnobDLB1) {
       inf_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
    fa_opt_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
    sa_opt_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
    const bool is_dlb1_hit =
      sa_lru_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);

    if (KnobULB2) {
      if (!is_dlb1_hit) {
           inf_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
        fa_opt_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
        sa_opt_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
        sa_lru_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
      }
    }
  }
  }
...
void StoreSingleLine(ADDRINT inst_addr, ADDRINT addr)
{
  ...
  if(use_bitmap) {
  if (KnobDLB1) {
       inf_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
    fa_opt_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
    sa_opt_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
    const bool is_dlb1_hit =
      sa_lru_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kStore);

    if (KnobULB2) {
      if (!is_dlb1_hit) {
           inf_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
        fa_opt_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
        sa_opt_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
        sa_lru_ulb2->AccessSingleLine(inst_addr, addr, BaseCache::kStore);
      }
    }
  }
  }
...
void FetchMultipleLines(ADDRINT inst_addr, UINT32 size)
{
  ...
  if(use_bitmap) {
  if (KnobILB1) {
       inf_ilb1->FetchMultipleLines(inst_addr, size);
    fa_opt_ilb1->FetchMultipleLines(inst_addr, size);
    sa_opt_ilb1->FetchMultipleLines(inst_addr, size);
    const bool is_ilb1_hit = sa_lru_ilb1->FetchMultipleLines(inst_addr, size);

    if (KnobULB2) {
      if (!is_ilb1_hit) {
           inf_ulb2->FetchMultipleLines(inst_addr, size);
        fa_opt_ulb2->FetchMultipleLines(inst_addr, size);
        sa_opt_ulb2->FetchMultipleLines(inst_addr, size);
        sa_lru_ulb2->FetchMultipleLines(inst_addr, size);
      }
    }
  }
  }
...
void FetchSingleLine(ADDRINT inst_addr)
{
  ...
  if(use_bitmap) {
  if (KnobILB1) {
       inf_ilb1->FetchSingleLine(inst_addr);
    fa_opt_ilb1->FetchSingleLine(inst_addr);
    sa_opt_ilb1->FetchSingleLine(inst_addr);
    const bool is_ilb1_hit = sa_lru_ilb1->FetchSingleLine(inst_addr);

    if (KnobULB2) {
      if (!is_ilb1_hit) {
           inf_ulb2->FetchSingleLine(inst_addr);
        fa_opt_ulb2->FetchSingleLine(inst_addr);
        sa_opt_ulb2->FetchSingleLine(inst_addr);
        sa_lru_ulb2->FetchSingleLine(inst_addr);
      }
    }
  }
  }
...
void PrintStats()
{
  ...
  if (KnobDLB1) {
    std::cerr << "L1 Data Bitmap (hit/miss)\t";
    PrintMissStats(dlb1);
  }
  if (KnobILB1) {
    std::cerr << "L1 Inst Bitmap (hit/miss)\t";
    PrintMissStats(ilb1);
  }
  if (KnobULB2) {
    std::cerr << "L2 Bitmap(hit/miss)\t\t";
    PrintMissStats(ulb2);
  }
...
void Instruction(INS ins, void *v)
{
  ...
  //find type of ins
  string insType = INS_Disassemble(ins);
  if (INS_IsCall(ins))
  {
	  ADDRINT address = INS_Address(ins);
	  string name = RTN_FindNameByAddress(address);
	  std::size_t found = name.find("QueueToBitmap");
	  std::size_t found2 = name.find("BitmapToQueue");
	  std::size_t found3 = name.find("BUStep");
	  std::size_t stop = name.find("PrintStep");
	  std::size_t stop2 = name.find("swap");
	  if(found != std::string::npos || found2 != std::string::npos /*|| found3 != std::string::npos*/) 
	  {
	  	//std::cerr << name << "\n";
		use_bitmap = "true";
	  }
	  else if(stop != std::string::npos /*|| stop2 != std::string::npos */)
	  {
		 //std::cerr << name << "\n";
		  use_bitmap = "false";
	  }
...
void PrintCacheConfig()
{
  ...
  if (KnobDLB1) {
    std::cerr << "L1 Data Bitmap (config)\t";
    PrintCacheConfigMain(DLB1);
  }
 if (KnobILB1) {
    std::cerr << "L1 Instruection Bitmap (config)\t";
    PrintCacheConfigMain(ILB1);
  }
  if (KnobULB2) {
    std::cerr << "L2 Bitmap (config)\t";
    PrintCacheConfigMain(ULB2);
  }
...
int main(int argc, char *argv[])
{
  ...
  // L1 Bitmap Data cache {{{ 
  if (KnobDLB1) {
    dlb1_output_inst.open(KnobDL1OutputInst.Value().c_str());
    dlb1_output_addr.open(KnobDL1OutputAddr.Value().c_str());
    dlb1_output_time.open(KnobDL1OutputTime.Value().c_str());
    dlb1_output_life.open(KnobDL1OutputLife.Value().c_str());

    inf_dlb1    = new l1_cache::InfCache(
        // 2nd and 4th doesn't really matter but make it same as real
        "Infinite L1 data cache",
    ...
    fa_opt_dlb1 = new l1_cache::OptCache(
        "Fully associative L1 data cache with Optimal replacement",
        KnobDLB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobDLB1CacheSize.Value() * KILO / KnobLineSize.Value(),
        KnobLookaheadDistance.Value(),
        KnobProcessWindow.Value());

    sa_opt_dlb1 = new l1_cache::OptCache(
        "Set associative L1 data cache with Optimal replacement",
        KnobDLB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobDLB1NumWays.Value(),
    ...
    sa_lru_dlb1 = new l1_cache::LruCache(
        "Set associative L1 data cache with LRU replacement",
        KnobDLB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobDL1NumWays.Value());
  ...
  // }}}
   ...
   // L1 Bitmap instruction cache {{{
  if (KnobILB1) {
    ilb1_output_inst.open(KnobIL1OutputInst.Value().c_str());
    ilb1_output_time.open(KnobIL1OutputTime.Value().c_str());
    ilb1_output_life.open(KnobIL1OutputLife.Value().c_str());

    inf_ilb1    = new l1_cache::InfCache(
        // 2nd and 4th doesn't really matter but make it same as real
        "Infinite L1 instruction cache",
        KnobILB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobILB1NumWays.Value());

    fa_opt_ilb1 = new l1_cache::OptCache(
        "Fully associative L1 instruction cache with Optimal replacement",
        KnobILB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobILB1CacheSize.Value() * KILO / KnobLineSize.Value(),
        KnobLookaheadDistance.Value(),
        KnobProcessWindow.Value());

    sa_opt_ilb1 = new l1_cache::OptCache(
        "Set associative L1 instruction cache with Optimal replacement",
        KnobILB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobILB1NumWays.Value(),
    ...
    sa_lru_ilb1 = new l1_cache::LruCache(
        "Set associative L1 instruction cache with LRU replacement",
        KnobILB1CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobILB1NumWays.Value());
  ...
  // L2 Bitmap cache {{{
  if (KnobULB2) {
    ulb2_output_inst.open(KnobUL2OutputInst.Value().c_str());
    ulb2_output_addr.open(KnobUL2OutputAddr.Value().c_str());
    ulb2_output_time.open(KnobUL2OutputTime.Value().c_str());
    ulb2_output_life.open(KnobUL2OutputLife.Value().c_str());

    inf_ulb2    = new l2_cache::InfCache(
        // 2nd and 4th doesn't really matter but make it same as real
        "Infinite L2 cache",
        KnobULB2CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobULB2NumWays.Value());

    fa_opt_ulb2 = new l2_cache::OptCache(
        "Fully associative L2 cache with Optimal replacement",
        KnobULB2CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobULB2CacheSize.Value() * KILO / KnobLineSize.Value(),
        KnobLookaheadDistance.Value(),
        KnobProcessWindow.Value());

    sa_opt_ulb2 = new l2_cache::OptCache(
        "Set associative L2 cache with Optimal replacement",
        KnobULB2CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobULB2NumWays.Value(),
    ...
    sa_lru_ulb2 = new l2_cache::LruCache(
        "Set associative L2 cache with LRU replacement",
        KnobULB2CacheSize.Value() * KILO,
        KnobLineSize.Value(),
        KnobULB2NumWays.Value());
