...
KNOB<UINT32> KnobDLB1NumWays(KNOB_MODE_WRITEONCE, "pintool",
	"dlb1_a", "8", "Cache associativity (1 for direct map) of DLB1");
KNOB<UINT32> KnobDLB1CacheSize(KNOB_MODE_WRITEONCE, "pintool",
	"dlb1_c", "64", "Cache size of DLB1 in kilobytes");
...
void LoadMultipleLines(ADDRINT inst_addr, ADDRINT addr, UINT32 size)
{
  ...
  if (KnobDLB1) {
	  bit_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kLoad);
  }
}
...
void StoreMultipleLines(ADDRINT inst_addr, ADDRINT addr, UINT32 size)
{
  ...
  if (KnobDLB1) {
	  bit_dlb1->AccessMultipleLines(inst_addr, addr, size, BaseCache::kStore);
  }
}
...
void LoadSingleLine(ADDRINT inst_addr, ADDRINT addr)
{
  ...
  if (KnobDLB1) {
	  bit_dlb1->AccessSingleLine(inst_addr, addr, BaseCache::kLoad);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  // L1 Data Bitmap cache {{{ 
  if (KnobDLB1) {
	  bit_dlb1 = new l1_cache::LruCache(
		  "Data L1 cache for bitmap",
